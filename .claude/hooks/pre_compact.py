...
def announce_pre_compact():
    """Announce pre-compact event using TTS."""
    try:
        tts_script = get_tts_script_path()
        if not tts_script:
            return  # No TTS scripts available
        
        message = "Preparing to compact chat history"
...
def main():
    try:
        ...
        # Announce pre-compact via TTS only if --notify flag is set
        if args.notify:
            announce_pre_compact()
